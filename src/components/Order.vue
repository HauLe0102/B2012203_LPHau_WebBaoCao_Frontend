...
<template>
    ...
    <form @submit.prevent id="form_addAuthor" style="display:none;width:100%;">
        ...
        <div class="mb-3">
            ...
            <input type="text" class="form-control" id="exampleInputEmail1" aria-describedby="emailHelp"
                placeholder="Vui lòng nhập vào tên tác giả..." v-model="AuthorInput.author_name" required>
        </div>

        <button @click="handleAddAuthor()" type="submit" class="btn btn-light fw-bold"
    ...
    &nbsp;
    <button type="button" class="btn btn-info text-white" @click="filterOrder('Đang vận chuyển')">Đang vận
        chuyển</button>
    &nbsp;
    ...
    <table class="table" border="1px" style="width:90vw;" id="table_manageAuthor">
        ...
        <tbody>
            <tr v-for="(item, index) in ManageOrder">
                ...
                <td v-else-if="item.statusOrder === 'Đã xử lý'" style="color:#0d6efd;font-weight:bold">{{
                    item.statusOrder }}</td>
                <td v-else-if="item.statusOrder === 'Đang vận chuyển'" style="color:#0dcaf0;font-weight:bold">{{
                    item.statusOrder }}</td>
                <td v-else-if="item.statusOrder === 'Đã giao hàng'" style="color:#198754;font-weight:bold">{{
                    item.statusOrder }}</td>
                <td v-else-if="item.statusOrder === 'Hủy đơn hàng'" style="color:#dc3545;font-weight:bold">{{
                    item.statusOrder }}</td>
                <td>
